...
__host__ __device__ unsigned int GETU32(const char *pt)
{
...
void AES_128_encrypt_CTR(unsigned int *out, const unsigned int *rk, unsigned int counter, uint32_t* in)
{
	...
	*/
	...
	// Use these for checking OneTblBytePermOri
	// out[0] = s0 ^ in[0];
	// out[1] = s1 ^ in[1];
	// out[2] = s2 ^ in[2];
	// out[3] = s3 ^ in[3];		
...
void AES_128_encrypt_CTR_pret3(unsigned int *out, const unsigned int *rk, unsigned int counter, uint32_t* in, uint32_t *pret2, uint32_t *pret3)
{
	...
	*/
	...
	// pret2[counter/65536] = t2;
	pret3[counter/16777216] = t3;	
}
...
__global__ void OneTblBytePermReuse(uint32_t *out, uint32_t* rk, uint32_t* t0G, uint32_t* in, uint32_t *pret3) {
	...
	// Each warp loads 32 values, total blockDim.x/SHARED_MEM_BANK_SIZE	warps are involved.
	uint32_t wid = threadIdx.x / SHARED_MEM_BANK_SIZE;
	for (uint32_t i = 0; i < SHARED_MEM_BANK_SIZE/(blockDim.x/TABLE_SIZE); i++) {
		t0S[wid + i* (TABLE_SIZE/SHARED_MEM_BANK_SIZE*(blockDim.x/TABLE_SIZE))][threadIdx.x%SHARED_MEM_BANK_SIZE] = t0G[wid + i*(TABLE_SIZE/SHARED_MEM_BANK_SIZE*(blockDim.x/TABLE_SIZE))];
	}	
	...
	for(int l=0; l<REPEAT; l++)
	{
		...
		s1 = 0 ^ rkS[1];
		s2 = 0 ^ rkS[2];
		// s3 = ((l*threadSize/REPEAT)*gridSize + tid) ^ rkS[3];
		// Only use 32-bit counter
		s3 = ((uint64_t) l*threadSize*gridSize/REPEAT + tid) ^ rkS[3];
		for (uint8_t r = 0; r < ROUND_COUNT_MIN_1; r++) {
				// Table based round function
			uint32_t rkStart = r * 4 + 4;
			// t0 = t0S[s0 >> 24][warpThreadIndex] ^ BytePerm(t0S[(s1 >> 16) & 0xFF][warpThreadIndex], SHIFT_1_RIGHT) ^ BytePerm(t0S[(s2 >> 8) & 0xFF][warpThreadIndex], SHIFT_2_RIGHT) ^ BytePerm(t0S[s3 & 0xFF][warpThreadIndex], SHIFT_3_RIGHT) ^ rkS[rkStart];
			// t1 = t0S[s1 >> 24][warpThreadIndex] ^ BytePerm(t0S[__byte_perm(s2, 0x00000000, 0x4562)][warpThreadIndex], SHIFT_1_RIGHT) ^ BytePerm(t0S[__byte_perm(s3, 0x00000000, 0x4561)][warpThreadIndex], SHIFT_2_RIGHT) ^ BytePerm(t0S[s0 & 0xFF][warpThreadIndex], SHIFT_3_RIGHT) ^ rkS[rkStart + 1];
			// t2 = t0S[s2 >> 24][warpThreadIndex] ^ BytePerm(t0S[(s3 >> 16) & 0xFF][warpThreadIndex], SHIFT_1_RIGHT) ^ BytePerm(t0S[(s0 >> 8) & 0xFF][warpThreadIndex], SHIFT_2_RIGHT) ^ BytePerm(t0S[s1 & 0xFF][warpThreadIndex], SHIFT_3_RIGHT) ^ rkS[rkStart + 2];
			// t3 = t0S[s3 >> 24][warpThreadIndex] ^ BytePerm(t0S[(s0 >> 16) & 0xFF][warpThreadIndex], SHIFT_1_RIGHT) ^ BytePerm(t0S[(s1 >> 8) & 0xFF][warpThreadIndex], SHIFT_2_RIGHT) ^ BytePerm(t0S[s2 & 0xFF][warpThreadIndex], SHIFT_3_RIGHT) ^ rkS[rkStart + 3];			
			t0 = t0S[s0 >> 24][warpThreadIndex] ^ BytePerm(t0S[__byte_perm(s1, 0x00000000, 0x4562)][warpThreadIndex], SHIFT_1_RIGHT) ^ BytePerm(t0S[__byte_perm(s2, 0x00000000, 0x4561) & 0xFF][warpThreadIndex], SHIFT_2_RIGHT) ^ BytePerm(t0S[s3 & 0xFF][warpThreadIndex], SHIFT_3_RIGHT) ^ rkS[rkStart];
			t1 = t0S[s1 >> 24][warpThreadIndex] ^ BytePerm(t0S[__byte_perm(s2, 0x00000000, 0x4562)][warpThreadIndex], SHIFT_1_RIGHT) ^ BytePerm(t0S[__byte_perm(s3, 0x00000000, 0x4561)][warpThreadIndex], SHIFT_2_RIGHT) ^ BytePerm(t0S[s0 & 0xFF][warpThreadIndex], SHIFT_3_RIGHT) ^ rkS[rkStart + 1];
			t2 = t0S[s2 >> 24][warpThreadIndex] ^ BytePerm(t0S[__byte_perm(s3, 0x00000000, 0x4562)][warpThreadIndex], SHIFT_1_RIGHT) ^ BytePerm(t0S[__byte_perm(s0, 0x00000000, 0x4561)][warpThreadIndex], SHIFT_2_RIGHT) ^ BytePerm(t0S[s1 & 0xFF][warpThreadIndex], SHIFT_3_RIGHT) ^ rkS[rkStart + 2];
			t3 = t0S[s3 >> 24][warpThreadIndex] ^ BytePerm(t0S[__byte_perm(s0, 0x00000000, 0x4562)][warpThreadIndex], SHIFT_1_RIGHT) ^ BytePerm(t0S[__byte_perm(s1, 0x00000000, 0x4561)][warpThreadIndex], SHIFT_2_RIGHT) ^ BytePerm(t0S[s2 & 0xFF][warpThreadIndex], SHIFT_3_RIGHT) ^ rkS[rkStart + 3];
		...
		s0 = __byte_perm(t0S[t0 >> 24][warpThreadIndex], 0x00000000, 0x1456) ^ __byte_perm(t0S[t1 >> 16& 0xff][warpThreadIndex], 0x00000000, 0x4156) ^ ((t0S[t2 >> 8 & 0xff][warpThreadIndex] & 0x0000ff00)) ^ __byte_perm(t0S[t3 & 0xff][warpThreadIndex], 0x00000000, 0x4561) ^ rkS[40];		
		s1 = __byte_perm(t0S[t1 >> 24][warpThreadIndex], 0x00000000, 0x1456)  ^ __byte_perm(t0S[t2 >> 16& 0xff][warpThreadIndex], 0x00000000, 0x4156) ^ ((t0S[t3 >> 8 & 0xff][warpThreadIndex] & 0x0000ff00)) ^ __byte_perm(t0S[t0 & 0xff][warpThreadIndex], 0x00000000, 0x4561) ^ rkS[41];		
		s2 = __byte_perm(t0S[t2 >> 24][warpThreadIndex],0x00000000, 0x1456) ^ __byte_perm(t0S[t3 >> 16& 0xff][warpThreadIndex], 0x00000000, 0x4156) ^ ((t0S[t0 >> 8 & 0xff][warpThreadIndex] & 0x0000ff00)) ^ __byte_perm(t0S[t1 & 0xff][warpThreadIndex], 0x00000000, 0x4561) ^ rkS[42];	
		s3 = __byte_perm(t0S[t3 >> 24][warpThreadIndex], 0x0000000, 0x1456) ^ __byte_perm(t0S[t0 >> 16& 0xff][warpThreadIndex], 0x00000000, 0x4156) ^ ((t0S[t1 >> 8 & 0xff][warpThreadIndex] & 0x0000ff00)) ^ __byte_perm(t0S[t2 & 0xff][warpThreadIndex], 0x00000000, 0x4561) ^ rkS[43];
		// s0 = ((t0S[t0 >> 24][warpThreadIndex] & 0x0000ff00) <<16)^ ((t0S[t1 >> 16& 0xff][warpThreadIndex] & 0x0000ff00) <<8) ^ ((t0S[t2 >> 8 & 0xff][warpThreadIndex] & 0x0000ff00)) ^ ((t0S[t3 & 0xff][warpThreadIndex] & 0x0000ff00) >>8) ^ rkS[40];
		// s1 = ((t0S[t1 >> 24][warpThreadIndex] & 0x0000ff00) <<16) ^ ((t0S[t2 >> 16& 0xff][warpThreadIndex] & 0x0000ff00) <<8) ^ ((t0S[t3 >> 8 & 0xff][warpThreadIndex] & 0x0000ff00)) ^ ((t0S[t0 & 0xff][warpThreadIndex] & 0x0000ff00) >>8) ^ rkS[41];
		// s2 = ((t0S[t2 >> 24][warpThreadIndex] & 0x0000ff00) <<16) ^ ((t0S[t3 >> 16& 0xff][warpThreadIndex] & 0x0000ff00) <<8) ^ ((t0S[t0 >> 8 & 0xff][warpThreadIndex] & 0x0000ff00)) ^ ((t0S[t1 & 0xff][warpThreadIndex] & 0x0000ff00) >>8) ^ rkS[42];
		// s3 = ((t0S[t3 >> 24][warpThreadIndex] & 0x0000ff00) <<16) ^ ((t0S[t0 >> 16& 0xff][warpThreadIndex] & 0x0000ff00) <<8) ^ ((t0S[t1 >> 8 & 0xff][warpThreadIndex] & 0x0000ff00)) ^ ((t0S[t2 & 0xff][warpThreadIndex] & 0x0000ff00) >>8) ^ rkS[43];

		// The global memory store is not coalesced, can we solve this?
		// tidStore = tid * 4 + (uint64_t)4*l*threadSize*gridSize/REPEAT;
		// out[tidStore] = s0 ^ in[tidStore];
		// out[tidStore +1] = s1^ in[tidStore +1];
		// out[tidStore +2] = s2^ in[tidStore +2];
		// out[tidStore +3] = s3^ in[tidStore +3];
		...
		tidStore = tid + (uint64_t)l*msgSize/4/REPEAT;
		uint32_t stride = (uint64_t)msgSize/4;
		out[tidStore] = s0^ in[tidStore];
		out[tidStore + stride] = s1^ in[tidStore + stride];
		out[tidStore + 2*stride] = s2^in[tidStore + 2*stride];
...
__global__ void OneTblBytePermReuseUnroll(uint32_t *out, uint32_t* rk, uint32_t* t0G, uint32_t* in, uint32_t *pret2, uint32_t *pret3) {
	...
	__shared__ uint32_t t0S[TABLE_SIZE][SHARED_MEM_BANK_SIZE];
	__shared__ uint32_t rkS[AES_128_KEY_SIZE_INT];
	__shared__ uint32_t s_pret2[2048];
	// This removes the bank conflicts in storing SM
	// Each warp loads 32 values, total blockDim.x/SHARED_MEM_BANK_SIZE	warps are involved.
	uint32_t wid = threadIdx.x / SHARED_MEM_BANK_SIZE;
	for (uint32_t i = 0; i < SHARED_MEM_BANK_SIZE/(blockDim.x/TABLE_SIZE); i++) {
	...
	// if(threadIdx.x < pret2Size)
		s_pret2[threadIdx.x] = pret2[threadIdx.x];
		s_pret2[threadIdx.x + 1024] = pret2[threadIdx.x + 1024];
	__syncthreads();
	...
	for(int l=0; l<REPEAT; l++)
	{
		...
		s1 = 0 ^ rkS[1];
		s2 = 0 ^ rkS[2];
		ctr = ((uint64_t) l*threadSize*gridSize/REPEAT + tid) ;
		s3 = ctr^ rkS[3];
		...
		// t2 = pret2[ctr/65536];
		t2 = s_pret2[ctr/65536];
		t3 = pret3[ctr/16777216];
		s0 = t0;	s1 = t1;	s2 = t2;	s3 = t3;

		for (uint8_t r = 1; r < 9; r+=4) {
				// Table based round function
			rkStart = r * 4 + 4;	
			t0 = t0S[s0 >> 24][warpThreadIndex] ^ BytePerm(t0S[__byte_perm(s1, 0x00000000, 0x4562)][warpThreadIndex], SHIFT_1_RIGHT) ^ BytePerm(t0S[__byte_perm(s2, 0x00000000, 0x4561) & 0xFF][warpThreadIndex], SHIFT_2_RIGHT) ^ BytePerm(t0S[s3 & 0xFF][warpThreadIndex], SHIFT_3_RIGHT) ^ rkS[rkStart];
			t1 = t0S[s1 >> 24][warpThreadIndex] ^ BytePerm(t0S[__byte_perm(s2, 0x00000000, 0x4562)][warpThreadIndex], SHIFT_1_RIGHT) ^ BytePerm(t0S[__byte_perm(s3, 0x00000000, 0x4561)][warpThreadIndex], SHIFT_2_RIGHT) ^ BytePerm(t0S[s0 & 0xFF][warpThreadIndex], SHIFT_3_RIGHT) ^ rkS[rkStart + 1];
